...
dataset_name = dataset_title.lower().replace(' ', '-')
    ...
    # Optional: prevent specific users from being added to the autogenerated
    # contributors list when the dataset is created or updated.
    # Use the user IDs returned by the CKAN API.
    data['untracked_contributors_ids'] = ['user-id-to-exclude']
