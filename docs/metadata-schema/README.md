...
If you need to exclude one or more users from being added as contributors during an API import or update, pass their user IDs in `untracked_contributors_ids` when calling `package_create` or `package_patch`.
...
{
    "id": "my-dataset",
    "untracked_contributors_ids": ["user-id-to-exclude"]
}
```
