...
import { fetchTagsFromDatasets } from "@/utils/fetchDatasetTags";
import type { Visualization } from "@/config/visualizations";
...
// Fetch tags at build time
export const getStaticProps: GetStaticProps<ShowroomProps> = async () => {
  ...
  console.log('🔄 Fetching tags for visualizations with auto-tags enabled...');

  // Fetch tags for visualizations marked with 'auto'
  const enhancedVisualizations = await Promise.all(
    VISUALIZATIONS.map(async (viz) => {
      if (viz.tags === 'auto') {
        console.log(`📊 Fetching tags for: ${viz.title}`);
        
        const datasetUrls = viz.datasets.map(ds => ds.url);
        const fetchedTags = await fetchTagsFromDatasets(datasetUrls);
        
        console.log(`✅ Found ${fetchedTags.length} tags for ${viz.id}:`, fetchedTags);
        
        // Return tags array or omit the property (don't use undefined)
        if (fetchedTags.length > 0) {
          ...
          // Omit tags property entirely instead of setting to undefined
          const { tags, ...vizWithoutTags } = viz;
          return vizWithoutTags;
        }
      }
      
      // For manual tags or no tags, return as-is but handle undefined
      if (viz.tags === undefined) {
        const { tags, ...vizWithoutTags } = viz;
        return vizWithoutTags;
      }
  ...
  return {
    ...
    revalidate: 3600, // Revalidate every hour (ISR - Incremental Static Regeneration)
  };
};

export default function ShowroomPage({ visualizations }: ShowroomProps) {
            ...
            >
              ...
              <div className="flex-shrink-0 border-b border-gray-200 bg-white px-4 py-3 md:px-6 md:py-4">
                ...
                <div className="mt-2 md:mt-3 flex flex-wrap items-center gap-2 md:gap-3">
                  {selectedVisualization.tags &&
                    Array.isArray(selectedVisualization.tags) &&
              ...
              <div className="flex-shrink-0 border-t border-gray-200 bg-gray-50 px-4 py-3 md:px-6 md:py-4">
                <h4 className="text-xs md:text-sm font-bold text-gray-900">Datasets used:</h4>
