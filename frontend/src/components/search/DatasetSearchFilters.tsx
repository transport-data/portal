...
export default function DatasetSearchFilters({
  ...
  filtersOptions?: Partial<{
    tags: {
      enabled: boolean;
      //  TODO: move filterNames into filterOptions
    };
    groups: {
      enabled: boolean;
    };
  ...
  const filterOptions = {
    tags: { enabled: true, ...ogFilterOptions?.tags },
  ...
  return (
    ...
    >
      <Form>
        ...
        {filterOptions.groups.enabled && (
          <section className="bg-white rounded-lg xl:p-8 p-4 mb-4 max-h-[400px] overflow-y-auto">
            <h1 className="font-bold pb-4">
              Refine by {`${filtersName?.group || 'Theme'}`}
            </h1>
